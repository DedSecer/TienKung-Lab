...
"""SAC+AMP Run Task Configuration.
...
- Actor/Critic hidden layers: [1024, 512], GELU activation
...
@configclass
class GaitCfg:
    """Gait configuration for running (trot-like pattern)."""
...
@configclass
class LiteRewardCfg:
    """Reward configuration for run task."""
...
@configclass
class TienKungSACRunFlatEnvCfg:
    """Environment configuration for SAC+AMP Run task."""
    ...
    commands: CommandsCfg = CommandsCfg(
        ...
        ranges=CommandRangesCfg(
            lin_vel_x=(-0.6, 1.5),  # Higher max velocity for running
            lin_vel_y=(-0.5, 0.5), 
            ang_vel_z=(-1.57, 1.57), 
    ...
    noise: NoiseCfg = NoiseCfg(
        add_noise=False,  # Disable noise initially for stability (as in paper)
...
class RslRlSacActorCriticCfg:
    """Configuration for SAC Actor-Critic network (from paper)."""
    class_name: str = "SACActorCritic"
    actor_hidden_dims: list = None  # [1024, 512] as per paper
    critic_hidden_dims: list = None  # [1024, 512] as per paper
    activation: str = "gelu"  # GELU as per paper
    init_noise_std: float = 1.0
    action_scale: float = 1.0
    
    def __post_init__(self):
        if self.actor_hidden_dims is None:
            self.actor_hidden_dims = [1024, 512]
        if self.critic_hidden_dims is None:
            self.critic_hidden_dims = [1024, 512]


@configclass
class RslRlSacAlgorithmCfg:
    """Configuration for SAC algorithm (from paper).
...
@configclass
class TienKungSACRunAgentCfg(RslRlOnPolicyRunnerCfg):
    """Agent configuration for SAC+AMP Run task."""
    ...
    # SAC Actor-Critic policy (paper architecture)
    policy = RslRlSacActorCriticCfg()
    
    # SAC algorithm (paper hyperparameters)
    algorithm = RslRlSacAlgorithmCfg()
    
    clip_actions = None
    save_interval = 100
    runner_class_name = "AmpOffPolicyRunner"  # Off-policy runner for SAC
    experiment_name = "sac_amp_run"
    run_name = ""
    logger = "tensorboard"
    neptune_project = "sac_amp_run"
    wandb_project = "sac_amp_run"
    resume = False
    load_run = ".*"
    load_checkpoint = "model_.*.pt"

    # AMP parameters (from paper Table II)
    amp_reward_coef = 0.3  # Modified: from 2.0 to 1.0 to 0.3 for stability (matching PPO settings)
    amp_motion_files = ["legged_lab/envs/tienkung/datasets/motion_amp_expert/run.txt"]
    amp_num_preload_transitions = 2_000_000  # Reduced from 2*10^7 to save memory (expert data loaded dynamically)
    amp_task_reward_lerp = 0.7  # Modified: from 0.4 to 0.6 to 0.7 (Task weight increased significantly)
